...
class MNISTSummation(Dataset):
    def __init__(
        ...
        dataset_len: int,
        train: bool = True,
        transform: Compose = MNIST_TRANSFORM,
    ):
        ...
        self.mnist = MNIST(
            DATA_ROOT, train=self.train, transform=self.transform, download=True
        )
        mnist_len = self.mnist.__len__()
        mnist_items_range = np.arange(0, mnist_len)

        items_len_range = np.arange(self.min_len, self.max_len + 1)
        items_len = np.random.choice(
            items_len_range, size=self.dataset_len, replace=True
        )
        self.mnist_items = []
        for i in range(self.dataset_len):
            self.mnist_items.append(
                np.random.choice(mnist_items_range, size=items_len[i], replace=True)
            )
...
if __name__ == "__main__":
    train_db = MNISTSummation(
        min_len=2, max_len=10, dataset_len=100000, train=True, transform=MNIST_TRANSFORM
    )
