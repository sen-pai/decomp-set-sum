...
from models import InvariantModel, SmallMNISTCNNPhi, SmallRho
...
def train_1_item(model, train_db, optimizer, item_number: int) -> float:
    ...
    the_loss = F.mse_loss(pred.view(-1), target)
...
test_db = MNISTSummation(min_len=5, max_len=50, dataset_len=10000, train=False)

the_phi = SmallMNISTCNNPhi()
the_rho = SmallRho(input_size=10, output_size=1)

model = InvariantModel(phi=the_phi, rho=the_rho)
if torch.cuda.is_available():
    model.cuda()
